...
export const params = {
  ...
  local: {
    scale: 0.48,
    // 14.0, the bottom of Beaufort 7, because the reference photograph moved.
    // Everything the long comment above says about 10.5 was tuned against Sea
    // of Thieves' fair-weather ocean; the current reference is a near-gale from
    // a ship's rail — foam blown in streaks, a wind sea visibly torn — and no
    // amount of foam work reads as that on top of a Beaufort-5 wave field. The
    // "whitecap snowfield" that made 16 m/s unusable was a failure of the FIXED
    // foam ration, not of the sea state: the ration now follows the wind (see
    // foamSeaState below), so the sea state is allowed to be what the photo is.
    windSpeed: 14.0, // m/s — Beaufort 7, the reference's sea state
...
// --- foam sea state ---------------------------------------------------------
// The foam budget as a FUNCTION of wind, replacing the hand-set constants that
// pinned the whole system to one sea state. Monahan & O'Muircheartaigh 1980:
// whitecap coverage W = 3.84e-6 * U10^3.41 — a cubic-plus law, so Beaufort 5 to
// Beaufort 7 is a 3-6x budget step, and no compile-time ration can express
// both ends. Everything here is expressed RELATIVE to the 10.5 m/s state every
// constant in maps.js was measured at, so at 10.5 m/s this function returns
// exactly the numbers that used to be hard-coded.
//
// The split matters more than the curve: the extra budget of a rising wind is
// almost entirely Stage-B residue (aged foam and bubble plume), not more
// simultaneous Stage-A caps — scaling cap events is what produced the
// documented "whitecap snowfield". So eligibility (how much sea MAY break)
// grows at a soft 0.45 power of the Monahan ratio, and the rest of the ratio
// lands on the residual injections (residScale) and on the lifetimes of the
// two long channels.
const FOAM_REF_WIND = 10.5; // m/s — the state the maps.js constants were measured at
// Inverse standard normal CDF (Acklam). The eligibility stencil thresholds a
// field renormalised to N(0.4089, 0.1328), so "this fraction of the sea is
// eligible" converts to a gate centre through the probit.
function probit(p) {
...
export function foamSeaState(p) {
  const U = Math.max(p.local.windSpeed, 0.5);
  const wf = Math.pow(U / FOAM_REF_WIND, 3.41); // Monahan ratio vs the reference state
  // Fraction of the surface the break stencil leaves eligible. FRAC_REF is
  // the exact Gaussian share of the old +1.085 sigma gate centre (0.553 on
  // N(0.4089, 0.1328)), so at the reference wind this returns the removed
  // constants to the digit; the 0.45 power keeps Stage A rare while the wind
  // climbs (16 m/s lands at ~26%, not the 59% a straight Monahan scaling
  // would ask of it).
  const FRAC_REF = 0.1389;
  const frac = clamp(FRAC_REF * Math.pow(wf, 0.45), 0.02, 0.42);
  const centre = 0.4089 + 0.1328 * probit(1 - frac);
  return {
    evtLo: centre - 0.017, // same 0.034 window width the constants had —
    evtHi: centre + 0.017, // "an event is an event" is a width, not a centre
    // What the eligibility growth did NOT carry of the Monahan ratio, applied
    // to the residual injections (trail, lace, remnant, aeration) in maps.js.
    residScale: clamp(wf / (frac / 0.15), 0.2, 5),
    // The two long channels live longer in a harder sea — more surfactant
    // stabilisation, deeper plumes. Weak powers, bounded.
    remTau: clamp(24 * Math.pow(wf, 0.30), 10, 70), // s — windrow remnant
    aerTau: clamp(28 * Math.pow(wf, 0.25), 15, 80), // s — subsurface bubble plume
    // Windage: residual foam skates downwind over the water it sits on at
    // roughly 3.5% of U10 (windage + Stokes drift). Drives the semi-Lagrangian
    // backtrace in maps.js. Baked at construction like the stencil lattice.
    advDrift: 0.035 * U,
  };
}
...
// tolRad: a FINER lattice (larger d, shorter period) must beat the reigning
// candidate by more than this to win. 0 keeps the classic best-angle pick;
// the windrow lane call passes ~2 degrees, because for a lane the PERIOD is
// the feature and a fraction of a degree of heading is worth nothing.
export function foamLattice(hx, hz, maxDen = 16, tolRad = 0) {
  let best = null;
  for (let d = 1; d <= maxDen; d++) {
    ...
    if (!best || err < best.err - Math.max(1e-9, tolRad)) best = { q, p, mag, err };
  }
